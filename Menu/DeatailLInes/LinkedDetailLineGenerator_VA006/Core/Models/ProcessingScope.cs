...
namespace Revit26_Plugin.LinkedDetailLineGenerator.VA006.Core.Models
{
    /// <summary>
    ...
    public partial class ProcessingScope : ObservableObject
    {
        ...
        /// <summary>OUTPUT GENERATION: when a Profile-group outer boundary loop comes
        /// out closed (after clipping/loop-closing), also creates a Revit Filled
        /// Region covering that loop's area, in addition to its Detail Lines. Reuses
        /// the mapping's own Detail Line Style name to look up a matching Filled
        /// Region Type by name; falls back to the first available Filled Region Type
        /// in the project (with a warning) if no name match is found.</summary>
        [ObservableProperty]
        private bool _createFilledRegions = false;

        /// <summary>CLEANUP: when enabled, Point-group mappings (Section 2c) are not
        /// processed at all for this run — no Circle/Rectangle markers are created,
        /// regardless of what is checked in Section 2c.</summary>
        [ObservableProperty]
        private bool _removePointMarkers = false;

        /// <summary>CLEANUP: after all Detail Lines and point markers for this run
        /// are created, deletes any created Detail Line with no connected neighbor
        /// (no shared endpoint within <see cref="LineJoinToleranceMm"/> tolerance
        /// among this run's own output) and any created point marker that does not
        /// touch a surviving line/loop from this run. Only touches elements created
        /// during this run — never pre-existing Detail Lines in the project.</summary>
        [ObservableProperty]
        private bool _removeIsolatedLinesAndPoints = false;
    }
