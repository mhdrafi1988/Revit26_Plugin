...
namespace Revit26_Plugin.AutomatedSectionPlacer.V001.Views
{
    /// <summary>
    /// Code-behind is intentionally minimal — all behavior is driven by
    /// AutomatedSectionPlacerViewModel via data binding and commands (MVVM).
    /// The window closes only when the ViewModel raises CloseRequested
    /// (wired in AutomatedSectionPlacerCommand), the user hits Esc/Close/Cancel,
    ...
    /// </summary>
    public partial class AutomatedSectionPlacerWindow : Window
    {
        public AutomatedSectionPlacerWindow()
        {
            InitializeComponent();
            LoadSharedStyles();
        }
        ...
        private void LoadSharedStyles()
        {
            ...
            {
                TaskDialog.Show("AutomatedSectionPlacer — Style Load Warning",
        ...
        {
            var vm = DataContext as AutomatedSectionPlacerViewModel;
            vm?.OnWindowClosing();
            vm?.Dispose();
        }
    }
