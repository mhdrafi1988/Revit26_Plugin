...
namespace Revit26_Plugin.AutomatedSectionPlacer.V001.ViewModels
{
    ...
    public partial class AutomatedSectionPlacerViewModel
    {
        // ---- Stage 1 state ----
        ...
        {
            IsBusy = true;
            BusyMessage = "Scanning plan view for section markers...";
            Logs.Add(new LogEntry(LogLevel.Info, "Requesting section-view scan from Revit."));

            _handler.PlanViewId = _planViewId;
            _handler.Request = AutomatedSectionPlacerRequest.LoadViews;
            _event.Raise();
        }

        /// <summary>Re-scans the same plan view for section markers — in case
        /// the user added/removed sections or changed placements since opening
        /// the tool. Does not re-prompt for a different plan view.</summary>
        [RelayCommand]
        private void RefreshViews() => RunLoadViews();

        /// <summary>Free-text entry for the Stage 1 toolbar's bulk scale-override control (e.g. "50" for 1:50).</summary>
        [ObservableProperty] private string _bulkScaleOverrideText = string.Empty;

        /// <summary>
        /// V001 (AutomatedSectionPlacer): applies BulkScaleOverrideText to
        /// every currently-selected (IsSelected) view's ScaleOverride —
        /// recomputes WidthMm/HeightMm immediately via ViewInfo's own
        /// OnScaleOverrideChanged, so Stage 2's packing sees the new size.
        /// A blank/invalid/zero-or-negative entry is a no-op (logged as a
        /// Warning) rather than silently clearing existing overrides.
        /// </summary>
        [RelayCommand]
        private void ApplyBulkScaleOverride()
        {
            if (!int.TryParse(BulkScaleOverrideText, out var scale) || scale <= 0)
            {
                Logs.Add(new LogEntry(LogLevel.Warning, $"Scale override: '{BulkScaleOverrideText}' is not a valid scale denominator — no changes applied."));
                return;
            }

            int count = 0;
            foreach (var v in AllViews.Where(v => v.IsSelected))
            {
                v.ScaleOverride = scale;
                count++;
            }
            Logs.Add(new LogEntry(LogLevel.Info, $"Scale override: set 1:{scale} on {count} selected view(s)."));
        }
        ...
        private void HandleLoadViewsCompleted()
        {
            ...
            PlanViewName = _handler.PlanViewName;

            ExistingSheets.Clear();
            foreach (var s in _handler.LoadedExistingSheets)
                ExistingSheets.Add(s);
            ...
            foreach (var vt in AllViews.Select(v => v.RevitViewType).Distinct())
            {
                ...
                {
                    // AutomatedSectionPlacer V001: unlike V222's project-wide
                    // browser (every ViewType starts unchecked there), this
                    // tool's whole point is "everything detected on the plan,
                    // ready to place" — every ViewType starts CHECKED on
                    // first-ever appearance, so all freshly-detected sections
                    // are selected by default (mixed placed/unplaced alike).
