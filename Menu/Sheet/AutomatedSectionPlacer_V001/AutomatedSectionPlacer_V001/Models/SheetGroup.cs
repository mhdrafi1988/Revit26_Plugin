...
namespace Revit26_Plugin.AutomatedSectionPlacer.V001.Models
{
    ...
    public partial class SheetGroup : ObservableObject
    {
        ...
        /// <summary>
        /// V220 NEW: "Sheet N: Type (Count)" — GeneratedName prefixed with this
        /// sheet's temporary/suggested number (SheetIndex, 1-based within its
        /// ViewType group). Same convention Stage 3's sheet badges already use
        /// (see the ItemsControl above the Stage 3 grid); exposed here as one
        /// shared property so every dropdown/list that identifies a sheet by
        /// name (Stage 2's Preview Sheet picker, Generated Sheets list) can
        /// bind DisplayMemberPath against it instead of each needing its own
        /// "Sheet {0}: {1}" StringFormat. Not the real Revit sheet number
        /// (AssignedSheetNumber) — that doesn't exist until Stage 3/4 creation.
        /// </summary>
        public string DisplayLabel => ExistingSheetId != null
            ? $"{GeneratedName} → {ExistingSheetLabel}"
            : $"Sheet {SheetIndex}: {GeneratedName}";

        /// <summary>
        /// V001 (AutomatedSectionPlacer): when set, this suggested sheet-group
        /// targets an EXISTING ViewSheet instead of creating a new one — the
        /// user picked it explicitly from the per-group "Target Sheet"
        /// dropdown (manual pick, confirmed — no automatic existing-sheet
        /// fill). Null (default) means "create new sheet", matching V222's
        /// only behavior. When set, ExecutePlaceViews skips ViewSheet.Create
        /// and places into the remaining free area below this existing
        /// sheet's current content instead.
        /// </summary>
        [ObservableProperty]
        private ElementId? _existingSheetId;

        /// <summary>Display text for the existing sheet ("A101 - Sections"), shown in DisplayLabel and the target-sheet dropdown.</summary>
        [ObservableProperty]
        private string? _existingSheetLabel;

        partial void OnExistingSheetIdChanged(ElementId? value) => OnPropertyChanged(nameof(DisplayLabel));

        /// <summary>
        /// Convenience wrapper for the Stage 3 "Target Sheet" ComboBox's
        /// SelectedItem — bind directly to this instead of juggling
        /// ExistingSheetId/ExistingSheetLabel separately from XAML. Null
        /// selection ("Create New Sheet", the ComboBox's first/default item)
        /// clears both. Holds the actual selected option instance (not a
        /// reconstructed copy), so WPF's SelectedItem reference-equality
        /// check against the ComboBox's ItemsSource keeps working.
        /// </summary>
        [ObservableProperty]
        private ExistingSheetOption? _targetExistingSheet;

        partial void OnTargetExistingSheetChanged(ExistingSheetOption? value)
        {
            ExistingSheetId = value?.SheetId;
            ExistingSheetLabel = value?.Label;
        }
        ...
        /// <summary>Display text for Stage 4's Status column: "Created"/"Placed on Existing"
        /// once CreatedSheetId is set, "Pending" beforehand.</summary>
        public string StatusText => CreatedSheetId == null
            ? "Pending"
            : (ExistingSheetId != null ? "Placed on Existing" : "Created");
        ...
        /// <summary>
        /// V220 NEW: which of the 7 packing strategies (RowFillStrategy) lays
        /// out THIS sheet's views. Pre-filled from the ViewModel's global
        /// default (AutomatedSectionPlacerViewModel.Stage2's
        /// DefaultFillStrategy) at the moment this SheetGroup is created
        /// during packing — acts as a per-sheet "template" the user can
        /// override afterward via the Generated Sheets list dropdown
        /// (confirmed: "one global option, each sheet has it on in data
        /// grid as template"). Changing this re-runs layout for this sheet
        /// only (see ReadingOrderPackingService.RepackSingleSheet).
        /// </summary>
        [ObservableProperty]
        private RowFillStrategy _fillStrategy;

        public SheetGroup(ViewType revitViewType, int sheetIndex, RowFillStrategy fillStrategy)
        {
            ...
            // (plus DisplayLabel, which composes GeneratedName) change
            // notifications whenever the collection changes (add/remove/
        ...
        /// <summary>DisplayLabel composes SheetIndex — CommunityToolkit.Mvvm only
        /// auto-raises change notification for SheetIndex itself, not for
        /// computed properties that read it, so this forwards the notification
        /// (same pattern as the Placements.CollectionChanged handler above,
        /// which does the equivalent for GeneratedName/TotalViewCount).</summary>
        partial void OnSheetIndexChanged(int value) => OnPropertyChanged(nameof(DisplayLabel));
