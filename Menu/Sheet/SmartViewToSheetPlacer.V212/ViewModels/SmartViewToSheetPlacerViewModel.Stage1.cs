...
namespace Revit26_Plugin.SmartViewToSheetPlacer.V212.ViewModels
{
    ...
    public partial class SmartViewToSheetPlacerViewModel
    {
        ...
        /// <summary>
        /// V212: guard flag set while THIS ViewModel is bulk-assigning
        ...
        /// <summary>
        /// V212: fires on ANY property change of ANY loaded ViewInfo — used
        ...
        /// bulk-selects/deselects every view of that type (V212 fix — previously this
        ...
        /// <summary>
        /// V212: bulk-sets IsSelected on every AllViews entry matching the given
        ...
        private void HandleLoadViewsCompleted()
        {
            ...
            foreach (var vt in AllViews.Select(v => v.RevitViewType).Distinct())
            {
                ...
                {
                    // V212: first-ever appearance of this type in this session
                    // (no prior state to restore, and there is no persisted
                    // cross-session View Type setting in SmartViewToSheetPlacerSettings) —
                    // confirmed with Rafi: default to Floor Plan checked only,
                    // every other type starts unchecked. Subsequent Refreshes
                    // within the same session will restore whatever the user
                    // sets from here on (handled by the branch above).
                    option.IsChecked = vt == ViewType.FloorPlan;
                }

                // V212 FIX: IsChecked is set above BEFORE we subscribe to
