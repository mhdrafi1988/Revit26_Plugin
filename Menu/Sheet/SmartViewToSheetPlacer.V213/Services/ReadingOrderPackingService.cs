...
namespace Revit26_Plugin.SmartViewToSheetPlacer.V213.Services
{
    /// <summary>
    ...
    ///
    /// V213: replaces GreedyRowPackingService (largest-first sort, fixed-gap
    /// only, force-placed oversized views). Confirmed changes vs V213:
    ...
    ///     replaces V213's single-pass speculative wrap check.
    ...
    public static class ReadingOrderPackingService
    {
        ...
        /// (never partially committed). This is the key V213 correction
        /// over V213's single-pass speculative wrap check.
        /// </summary>
            ...
            ViewGroupGapSettings gapSettings)
        {
            ...
            foreach (var row in rows)
            {
                ...
                // V213 UPDATE: bottom-align each view within its row, per Rafi's
                // confirmation — row height is still the tallest view in the
                // row (unchanged), but a shorter view's Y offset is now
                // rowTop + (rowHeight - v.HeightMm) instead of just rowTop, so
                // its BOTTOM edge touches the row's bottom (empty space now
                // appears ABOVE a shorter view, not below it as before).
                if (gapSettings.GapStyle == GapStyle.Fixed)
                {
                    double cursorX = 0;
                    foreach (var v in row)
                    {
                        double vTop = rowTop + (rowHeight - v.HeightMm);
                        AddPlacement(sheet, v, cursorX, vTop, positionsInOrder, ref placedCount);
                ...
                else // EvenGap
                {
                    if (row.Count == 1)
                    {
                        ...
                        double vTop = rowTop + (rowHeight - row[0].HeightMm);
                        AddPlacement(sheet, row[0], Math.Max(0, centerX), vTop, positionsInOrder, ref placedCount);
                    }
                    else
                    {
        ...
        /// V213: takes the target sheet's ViewGroupGapSettings instead of
