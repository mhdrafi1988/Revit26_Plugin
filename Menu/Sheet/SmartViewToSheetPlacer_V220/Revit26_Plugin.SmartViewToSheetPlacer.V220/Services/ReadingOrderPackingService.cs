    ...
    public static class ReadingOrderPackingService
    {
            ...
            List<ViewInfo> failedToPlaceOut)
        {
            ...
            while (remaining.Count > 0 && sheetIndex <= maxSheetsSafetyValve)
            {
                ...
                var stillUntouched = remaining.Where(v => !candidates.Contains(v)).ToList();
            ...
            RowFillStrategy strategy)
        {
            ...
            var placedOut = new List<(ViewInfo View, double OffsetXMm, double OffsetYMm)>();
            List<ViewInfo> unplacedOut;
                ...
                case RowFillStrategy.MaxRects:
                    {
                        ...
                        foreach (var p in r.Placed)
                            placedOut.Add((p.Item, p.X * FeetToMm, p.Y * FeetToMm));
                        unplacedOut = r.Unplaced;
                        break;
                    }
