...
using Revit26_Plugin.APUS_V314.Commands;
...
namespace Revit26_Plugin.APUS_V314.ViewModels
{
    ...
    public class AutoPlaceSectionsViewModel : BaseViewModel
    {
        ...
        public IRelayCommand EmergencyPlaceCommand { get; }
        public IRelayCommand DirectPlaceCommand { get; }
            ...
            System.Collections.Generic.List<string> preloadedPlacementScopes)
        {
            ...
            EmergencyPlaceCommand = new RelayCommand(ExecuteEmergencyPlacement, () => CanPlace);
            DirectPlaceCommand = new RelayCommand(ExecuteDirectPlacement, () => CanPlace);
        ...
        private void ExecuteEmergencyPlacement()
        {
            LogWarning("🚨 EMERGENCY PLACEMENT - ONE SECTION ONLY");
            ...
            if (selected.Any())
            {
                LogInfo($"Emergency placing: {selected[0].ViewName}");

                _placementHandler.SectionsToPlace = selected;
                _placementHandler.ViewModel = this;
                _placementEvent.Raise();
            }
            else
            {
                LogWarning("⚠️ No sections selected for emergency placement");
            }
        }

        private void ExecuteDirectPlacement()
        {
            LogWarning("⚡ DIRECT PLACEMENT - BYPASSING EXTERNAL EVENT");
            LogWarning("⚠️ This will run in UI thread - use with caution");
            ...
            if (!selected.Any())
            {
                LogWarning("⚠️ No sections selected for direct placement");
                return;
            }

            if (SelectedTitleBlock == null)
            {
                LogError("❌ No title block selected.");
                return;
            }

            try
            {
                var handler = new SectionPlacementHandler
                {
                    ViewModel = this,
                    SectionsToPlace = selected
                };

                handler.Execute(_uidoc.Application);
            }
            catch (Exception ex)
            {
                LogError($"❌ Direct placement failed: {ex.Message}");
            }
        }
        ...
        private void ExecuteRefresh()
        {
            ...
            try
            {
                ...
                var commandId = RevitCommandId.LookupCommandId("Custom.APUS_V314.AutoPlaceSections");
