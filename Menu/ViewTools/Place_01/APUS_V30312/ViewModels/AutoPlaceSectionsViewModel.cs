...
using Revit26_Plugin.APUS_V312.ExternalEvents;
...
namespace Revit26_Plugin.APUS_V312.ViewModels
{
    public class AutoPlaceSectionsViewModel : INotifyPropertyChanged
    {
        ...
        {
            return !IsPlacing && FilteredSections.Cast<SectionItemViewModel>().Any(x => x.IsSelected);
        }
        ...
        private void ExecutePlacement()
        {
            ...
            if (!toPlace.Any())
            {
                LogWarning("No sections passed filtering.");
                return;
            }
            ...
            {
                LogEntries.Clear();
                LogInfo($"Starting placement of {toPlace.Count} selected sections...");
            });

            // Setup handler and raise event
            AutoPlaceSectionsEventManager.Handler.ViewModel = this;
            AutoPlaceSectionsEventManager.Handler.SectionsToPlace = toPlace;
