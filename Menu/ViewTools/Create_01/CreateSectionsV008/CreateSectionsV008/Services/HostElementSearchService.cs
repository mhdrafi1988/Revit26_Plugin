    ...
    public class HostElementSearchService
    {
            ...
            HostSourceSelection source)
        {
            ...
            if (source.FloorHost)
                CollectFromDocument(_doc, Transform.Identity, midPoint, lineZ, thresholdFt,
                    BuiltInCategory.OST_Floors, results);

            if (source.RoofHost)
                CollectFromDocument(_doc, Transform.Identity, midPoint, lineZ, thresholdFt,
                    BuiltInCategory.OST_Roofs, results);

            if (source.FloorLinked)
                CollectFromLinks(midPoint, lineZ, thresholdFt, BuiltInCategory.OST_Floors, results);

            if (source.RoofLinked)
                CollectFromLinks(midPoint, lineZ, thresholdFt, BuiltInCategory.OST_Roofs, results);
            ...
            List<CandidateHostElement> results)
        {
            ...
            foreach (var el in collector)
            {
                ...
                results.Add(new CandidateHostElement(el, bb, horizontalDist));
            }
        }
