...
namespace Revit26_Plugin.RoofEdgeAroundSections.V003
{
    ...
    public static class RoofEdgeBucketingEngine
    {
        ...
        /// <summary>
        /// Collects every curve from the roof's ACTUAL built solid geometry (bottom-most
        /// near-horizontal face's edge loops) — not the flat sketch.
        ///
        /// FootPrintRoof.GetProfiles() was tried first but rejected: it returns the
        /// original sketch curves, which always sit flat at the roof's Level + Base
        /// Offset elevation, regardless of slope arrows or shape-edited sub-elements.
        /// On any sloped roof this put every section at the sketch's flat elevation
        /// instead of the real, per-edge elevation of the finished roof surface —
        /// confirmed as the root cause of sections landing at the wrong height.
        /// Reading the real solid via get_Geometry() tracks the built roof, including
        /// slope and shape edits, for every RoofBase-derived type.
        /// </summary>
            ...
            try
            {
                ...
                if (geomElem != null)
                {
                    foreach (GeometryObject obj in geomElem)
                    {
                        ...
                        {
                            // Use the bottom-most near-horizontal face's edge loop as the
                            // boundary — this follows the real, current roof surface
                            // (slope/shape edits included), not the flat original sketch.
