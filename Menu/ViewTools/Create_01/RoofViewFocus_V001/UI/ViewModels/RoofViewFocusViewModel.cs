...
using Revit26_Plugin.RoofViewFocus.V001.Infrastructure.ExternalEvents;
using Revit26_Plugin.Shared.Models;

namespace Revit26_Plugin.RoofViewFocus.V001.UI.ViewModels
{
    /// <summary>
    ...
    {
        /// <summary>Log folder â€” asked once per Revit session, then reused.</summary>
        private static string? _sessionLogFolder;
            ...
            int ignoredCount)
        {
            ...
            AddLog(LogLevel.Info, $"Settings loaded â€” margins {ViewMarginText} / {AnnotationMarginText} mm, offset {DefaultOffsetText} mm");
        }
        ...
        /// <summary>Set by the window; returns a chosen folder or null when cancelled.</summary>
        ...
        private void ExportLog()
        {
            ...
            try
            {
                string path = RoofViewFocusLogExporter.Write(folder, Log);
        ...
        private void AutoSaveLog()
        {
            ...
            if (folder == null)
            {
                AddLog(LogLevel.Warning, "Log auto-save skipped â€” no folder chosen");
                return;
            }
        ...
        {
            if (!string.IsNullOrEmpty(_sessionLogFolder) && Directory.Exists(_sessionLogFolder))
                return _sessionLogFolder;

            string? picked = PickFolder?.Invoke();
            if (!string.IsNullOrEmpty(picked))
                _sessionLogFolder = picked;
            return picked;
        }
