...
namespace Revit26_Plugin.SheetAutoRearrange.V011.Core.Services
{
    ...
    public class TitleBlockDetectionService
    {
        ...
        public TitleBlockDetectionResult Detect(Document doc, ViewSheet sheet, GapSettings gapSettings)
        {
            ...
            if (insetMax.X <= insetMin.X || insetMax.Y <= insetMin.Y)
            {
                ...
                result.Region = new PlaceableRegion(TitleBlockDetectionMode.Detected, rawRect, null,
                    "Detected (margins too large — ignored)");
                return result;
            }

            var rect = new RectFeet(insetMin, insetMax);
            result.Region = new PlaceableRegion(TitleBlockDetectionMode.Detected, rect, null, "Detected");
            return result;
        }
