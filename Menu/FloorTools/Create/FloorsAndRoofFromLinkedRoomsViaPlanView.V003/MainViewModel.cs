...
namespace Revit26_Plugin.FloorsAndRoofFromLinkedRoomsViaPlanView.V003
{
    public partial class MainViewModel : ObservableObject
    {
        ...
        public MainViewModel(Document hostDoc, ViewPlan planView, Level targetLevel, string levelFallbackNote,
            RunCreateElementsExternalEventHandler handler, ExternalEvent externalEvent)
        {
            _hostDoc = hostDoc;
            _planView = planView;
            _handler = handler;
            ...
            if (!string.IsNullOrEmpty(levelFallbackNote))
                AddLog(LogLevel.Warning, levelFallbackNote);
        }
        ...
        {
            // NewFootPrintRoof rejects roof types whose compound structure is null
            // (sloped glazing and similar) — passing one throws a bare ArgumentNullException
            // from deep inside the API. Filter them out so the dropdown only offers roof
            // types that can actually host a footprint roof.
            var types = new FilteredElementCollector(_hostDoc)
            ...
            if (RoofTypes.Count == 0)
                AddLog(LogLevel.Warning,
                    "No usable roof types found — all roof types in this model have no compound structure (e.g. sloped glazing), which cannot host a footprint roof.");
        }
        ...
        private void ExportLogs()
        {
            ...
            {
                AddLog(LogLevel.Error, $"Export failed: {ExceptionFormatting.Full(ex)}");
            }
        }
