...
public abstract class LinkedMechanicalElementProcessor
{
    ...
    /// <summary>One collector pass: source keys for which geometry already exists.
    /// Loaded once per run/preview instead of re-querying per element.</summary>
    protected abstract HashSet<string> LoadExistingSourceKeys(Document hostDoc, ViewPlan view);
        ...
        OperationSummary summary)
    {
        ...
        HashSet<string> existingKeys = LoadExistingSourceKeys(hostDoc, view);
        ...
        Document hostDoc, ViewPlan view, RevitLinkInstance selectedLink, string selectedFamilyName)
    {
        ...
        return CollectVisibleLinkedMechanicalEquipment(hostDoc, view, selectedLink)
            ...
            .Count(e => !existingKeys.Contains(CircleIdentityStorage.BuildSourceKey(selectedLink, e)));
    }
