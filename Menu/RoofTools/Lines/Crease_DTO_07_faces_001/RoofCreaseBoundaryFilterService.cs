// ==================================
// File: RoofCreaseBoundaryFilterService.cs
// ==================================
...
{
    /// <summary>
    /// Removes ALL boundary creases using polygon containment.
    /// This is the ONLY robust method.
    /// </summary>
    public class RoofCreaseBoundaryFilterService
    {
            ...
            IList<IList<IList<XYZ>>> holePolygons2d)
        {
            ...
            {
                XYZ mid = (crease.GetEndPoint(0) + crease.GetEndPoint(1)) * 0.5;

                bool insideOuter = outerPolygons2d.Any(p => IsPointInsidePolygon(mid, p));
                bool insideHole = holePolygons2d.Any(hs => hs.Any(h => IsPointInsidePolygon(mid, h)));

                if (insideOuter && !insideHole)
                    result.Add(crease);
            }

            _log.Info($"Internal creases kept: {result.Count}/{creaseLines2d.Count}");
            return result;
        }

        // Ray casting algorithm (robust, standard)
