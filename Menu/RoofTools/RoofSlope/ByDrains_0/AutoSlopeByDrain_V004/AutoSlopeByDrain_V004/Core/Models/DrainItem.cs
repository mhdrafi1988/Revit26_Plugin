...
namespace Revit26_Plugin.AutoSlopeByDrain.V004.Core.Models
{
    ...
    {
        // V003: added INotifyPropertyChanged so a direct grid checkbox click
        // (not just Select All/None/Invert) notifies the ViewModel to update
        // the selected count and re-evaluate the Run button.
                        ...
                        double width, double height, string shapeType, ElementId id = null)
        {
            ...
            {
                // BUGFIX: previously CenterPoint stayed null whenever no existing
                // SlabShapeVertex happened to sit within 5mm of this specific opening
                // (common — Revit doesn't auto-place shape-editor points at drain
                // openings). A null CenterPoint made RemoveDuplicateDrains misbehave
                // and could collapse every opening but the first down to just one.
