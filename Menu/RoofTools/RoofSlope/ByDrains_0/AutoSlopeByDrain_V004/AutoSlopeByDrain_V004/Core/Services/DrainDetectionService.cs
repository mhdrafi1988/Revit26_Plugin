...
namespace Revit26_Plugin.AutoSlopeByDrain.V004.Core.Services
{
    public class DrainDetectionService
    {
        ...
        private int _dbgSolidsSeen, _dbgFacesTotal, _dbgFacesUpward, _dbgLoopsSeen,
                    _dbgRejectedDegenerate, _dbgRejectedSize, _dbgRejectedException, _dbgAccepted;
        ...
        public List<DrainItem> DetectDrainsFromRoof(RoofBase roof, Face ignoredTopFace, List<SlabShapeVertex> allShapeVertices = null, Action<LogEntry> log = null)
        {
            ...
            _dbgSolidsSeen = _dbgFacesTotal = _dbgFacesUpward = _dbgLoopsSeen = 0;
            _dbgRejectedDegenerate = _dbgRejectedSize = _dbgRejectedException = _dbgAccepted = 0;
            ...
            try
            {
                ...
                if (geom == null)
                {
                    log?.Invoke(new LogEntry(LogLevel.Warning, "DRAIN-DEBUG: roof.get_Geometry() returned null — no geometry to scan."));
                    return drains;
                }
                ...
                foreach (GeometryObject go in geom)
                {
                    ...
                    {
                        _dbgSolidsSeen++;
                        _dbgFacesTotal += solid.Faces.Size;

                        // Only upward-facing roof surfaces (no bottoms)
                        var topFaces = GetUpwardFaces(solid, 0.1).ToList(); // normal.Z ≥ 0.1 ⇒ facing up (≈6° slope)
                        _dbgFacesUpward += topFaces.Count;

                        foreach (var face in topFaces)
                            drains.AddRange(DrainsFromFaceLoops(face, log));
                    }
                    else if (log != null && go is GeometryInstance)
                    {
                        log.Invoke(new LogEntry(LogLevel.Warning,
                            "DRAIN-DEBUG: skipped a GeometryInstance in roof geometry (nested/family geometry is not scanned — solids only)."));
                    }
                }
                ...
                log?.Invoke(new LogEntry(LogLevel.Info,
                    $"DRAIN-DEBUG summary: {_dbgSolidsSeen} solid(s), {_dbgFacesTotal} total face(s), {_dbgFacesUpward} upward-facing. " +
                    $"{_dbgLoopsSeen} inner loop(s) examined -> {_dbgAccepted} accepted, " +
                    $"{_dbgRejectedSize} rejected (size out of 5-2000mm range), " +
        ...
        private List<DrainItem> DrainsFromFaceLoops(Face face, Action<LogEntry> log = null)
        {
            ...
            for (int i = 1; i < face.EdgeLoops.Size; i++)
            {
                ...
                var drain = CreateDrainFromEdgeLoop(edgeLoop, face, log);
                if (drain != null)
                {
                    _dbgAccepted++;
                    list.Add(drain);
        ...
        private DrainItem CreateDrainFromEdgeLoop(EdgeArray loop, Face face, Action<LogEntry> log = null)
        {
            try
            {
                ...
                if (pts.Count < 3)
                {
                    ...
                    log?.Invoke(new LogEntry(LogLevel.Warning,
                        $"DRAIN-DEBUG: loop rejected — only {pts.Count} boundary point(s) found (degenerate/unreadable edge loop)."));
                    return null;
                }
                ...
                // Validate size range
                if (w < 5 || h < 5 || w > 2000 || h > 2000)
                {
                    _dbgRejectedSize++;
                    var cx = (minX + maxX) / 2 * 304.8;
                    var cy = (minY + maxY) / 2 * 304.8;
                    log?.Invoke(new LogEntry(LogLevel.Warning,
                        $"DRAIN-DEBUG: loop rejected — bounding box {w:F1}x{h:F1} mm (near {cx:F0},{cy:F0} mm) is outside the allowed 5-2000mm range."));
                    return null;
                }
        ...
        private List<DrainItem> RemoveDuplicateDrains(List<DrainItem> drains)
        {
            ...
            {
                // Defensive guard: CenterPoint should never be null now (see DrainItem
                // bugfix), but this prevents a single bad entry from throwing and
                // silently collapsing every remaining drain into "just one" again.
                if (d?.CenterPoint == null) continue;

                if (!unique.Any(x => x.CenterPoint != null && x.CenterPoint.DistanceTo(d.CenterPoint) < tol))
                    unique.Add(d);
            }
            return unique;
        }
