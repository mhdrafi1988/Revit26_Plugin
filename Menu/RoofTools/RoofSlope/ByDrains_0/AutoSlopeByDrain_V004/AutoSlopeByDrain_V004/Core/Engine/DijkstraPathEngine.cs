...
namespace Revit26_Plugin.AutoSlopeByDrain.V004.Core.Engine
{
    ...
    public class DijkstraPathEngine
    {
        ...
        // V004 addition: ByDrain exposes a user-facing "Path Samples" field
        // (unlike ByPoint, which uses a fixed dynamic sample count). This keeps
        // that control meaningful instead of silently ignoring it once the
        // tangent-arc engine took over edge validation.
        ...
        ///     it lies on the roof face — comes from ByDrain's UI "Path Samples" field.
        ...
        // CSV export / log ("nearest drain #3, 200x100mm") — these two methods walk
        // the same _lastPred chain DescribePathEdgeTypes already uses, just for a
        // different purpose. Neither touches the tangent/arc weight calculation.
        ...
        /// include intermediate tangent points on an arc for TangentRoute hops, same
        /// level of detail ByDrain's previous straight-line-only path solver gave.
        /// Must be called after ComputeAllDistances.
        /// </summary>
