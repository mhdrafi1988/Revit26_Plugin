...
using Revit26_Plugin.MultiRoofSlopeByDrain.V010.Infrastructure.ExternalEvents;
...
namespace Revit26_Plugin.MultiRoofSlopeByDrain.V010.UI.ViewModels
{
    public partial class AutoSlopeDrainViewModel : ObservableObject
    {
        ...
        // NEW (V010): Cancelled added, per Rafi's confirmed Cancel decision.
        ...
        [NotifyPropertyChangedFor(nameof(RunTimeRangeDisplay))]
        ...
        /// <summary>NEW (V010). Done OR Cancelled — used only to gate display of real numbers (metrics/timing); NOT used for Run-button re-enablement (see IsComplete), so cancelling never permanently locks the Run button.</summary>
        ...
        /// <summary>NEW (V010). "36 s" / "1 m 12 s" / "&lt;1 s", or "N/A" before this run's overall duration is known.</summary>
        public string RunDurationDisplay => IsFinished ? FormatDuration(RunDuration_sec) : "N/A";
        ...
        /// <summary>NEW (V010). Wall-clock start of this batch's run, e.g. "14:32:05". Set once the run finishes.</summary>
        [ObservableProperty]
        [NotifyPropertyChangedFor(nameof(RunTimeRangeDisplay))]
        private string runStartTime = "—";

        /// <summary>NEW (V010). Wall-clock end of this batch's run, e.g. "14:32:41". Set once the run finishes.</summary>
        [ObservableProperty]
        [NotifyPropertyChangedFor(nameof(RunTimeRangeDisplay))]
        private string runEndTime = "—";

        /// <summary>NEW (V010). "14:32:05 → 14:32:41" shown as the Run Duration metric card's caption, or "—" before a run finishes.</summary>
        public string RunTimeRangeDisplay => IsFinished ? $"{RunStartTime} → {RunEndTime}" : "—";

        // ── Progress / Cancel — NEW (V010), per Rafi's confirmed decisions ─────
        ...
        public AutoSlopeDrainViewModel(UIDocument uidoc, UIApplication app, List<RoofData> roofDataList)
        {
            ...
            int tabIndex = 1;
            var defaultSelectionLogs = new List<string>();
            foreach (var roofData in roofDataList)
            {
                ...
                // NEW (V010): log each roof's default circle selection right after
                // its tab is built, before the "Detected N drain opening(s)" summary
                // line below — deferred into a list rather than logged inline so it
                // reads as "here's what got auto-selected" grouped right before the
                // overall detection summary, not interleaved with tab construction.
                if (!string.IsNullOrEmpty(tab.DefaultSelectionSummary))
                    defaultSelectionLogs.Add(roofDataList.Count == 1
                        ? tab.DefaultSelectionSummary
                        : $"[{tab.RoofName}] {tab.DefaultSelectionSummary}");
            }
            ...
            foreach (var summary in defaultSelectionLogs)
                AddLog(new LogEntry(LogLevel.Info, summary));
        ...
        private void RunAutoSlope()
        {
            ...
            // NEW (V010): fresh CancellationTokenSource shared by every roof payload
            ...
            // NEW (V010), per Rafi's confirmed multi-roof decision: one continuous
            ...
            foreach (var tab in runnableTabs)
            {
                ...
                roofPayloads.Add(new AutoSlopeDrainPayload
                {
                    ...
                    // NEW (V010): called synchronously on the SAME thread as this run
            ...
            AutoSlopeDrainHandler.MultiPayload = new AutoSlopeDrainMultiPayload
            {
                ...
                OnAllCompleted = roofResults =>
                {
                    Application.Current.Dispatcher.BeginInvoke(new Action(() =>
                    {
                        ...
                        RunStartTime = overallStartTime.ToString("HH:mm:ss");
                        RunEndTime = overallEndTime.ToString("HH:mm:ss");
                        ...
                        if (succeeded.Count == 0)
                        {
                            ...
                            RunDuration_sec = (int)(overallEndTime - overallStartTime).TotalSeconds;
