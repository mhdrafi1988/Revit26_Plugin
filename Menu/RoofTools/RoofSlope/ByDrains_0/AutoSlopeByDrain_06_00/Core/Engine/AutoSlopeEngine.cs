...
using Revit26_Plugin.AutoSlopeByDrain_06_00;
...
using Revit26_Plugin.AutoSlopeByDrain_06_00.Core.Models;
using Revit26_Plugin.AutoSlopeByDrain_06_00.Core.Parameters;
using Revit26_Plugin.AutoSlopeByDrain_06_00.Infrastructure.Helpers;
using System;
...
namespace Revit26_Plugin.AutoSlopeByDrain_06_00.Core.Engine
{
    public static class AutoSlopeEngine
    {
        public static void Execute(UIApplication app, AutoSlopePayload data)
        {
            ...
            // Find drain indices - UPDATED to use DrainItems if available
            HashSet<int> drainIndices = new();

            if (data.DrainItems != null && data.DrainItems.Count > 0)
            {
                // Use drain items with corner points for better accuracy
                for (int i = 0; i < vertices.Count; i++)
                {
                    foreach (DrainItem drain in data.DrainItems.Where(d => d.IsSelected))
                    {
                        if (IsVertexInDrainArea(vertices[i].Position, drain))
                        {
                            drainIndices.Add(i);
                            break;
                        }
                ...
                data.Log(LogColorHelper.Cyan($"Found {drainIndices.Count} vertices within selected drain areas"));
            }
            else
            {
                // Fallback to original method with center points
                for (int i = 0; i < vertices.Count; i++)
                {
            ...
            data.Vm.DrainCount = data.DrainItems?.Count(d => d.IsSelected) ?? data.DrainPoints.Count;
            ...
            data.Log(LogColorHelper.Cyan($"Drain Count        : {data.DrainItems?.Count(d => d.IsSelected) ?? data.DrainPoints.Count}"));
        ...
        private static bool IsVertexInDrainArea(XYZ vertexPos, DrainItem drain)
        {
            double halfWidth = (drain.Width / 304.8) / 2;
            double halfHeight = (drain.Height / 304.8) / 2;

            return vertexPos.X >= drain.CenterPoint.X - halfWidth &&
                   vertexPos.X <= drain.CenterPoint.X + halfWidth &&
                   vertexPos.Y >= drain.CenterPoint.Y - halfHeight &&
                   vertexPos.Y <= drain.CenterPoint.Y + halfHeight;
        }
