...
// Namespace: Revit26_Plugin.AutoSlopeByPoint.V07.Core.Engine
//
...
using Revit26_Plugin.AutoSlopeByPoint.V07.Core.Parameters;
using Revit26_Plugin.AutoSlopeByPoint.V07.Infrastructure.Helpers;
using System;
...
namespace Revit26_Plugin.AutoSlopeByPoint.V07.Core.Engine
{
    public static class AutoSlopeEngine
    {
        public static void Execute(UIApplication app, AutoSlopePayload data)
        {
            ...
            // ── Single transaction wraps Reset + Apply ───────────────────────
            // FIX (V07): Reset and Apply used to be two separate committed
            // transactions, with several abort guards (top face, no drain
            // points, no matching vertices) running in between. Because those
            // guards returned without throwing, the outer TransactionGroup in
            // AutoSlopeHandler had nothing to roll back, so a "failed" run
            // could still leave the roof's vertices permanently flattened to
            // zero. Everything destructive now lives inside one transaction;
            // every guard below calls tx.RollBack() before returning, so a
            // failed run leaves the roof exactly as it was found.
            List<SlabShapeVertex> vertices = null;
            List<XYZ> finalDrainPoints = null;
            int processed = 0;
            int skipped = 0;
            double maxPathFt = 0;
            var vertexDataList = new List<VertexData>();
            Stopwatch sw = Stopwatch.StartNew();
            double drainBaselineZFt = 0;
            ...
            using (Transaction tx = new Transaction(doc, "Apply AutoSlope"))
            {
                ...
                finalDrainPoints = data.DrainPoints ?? new List<XYZ>();
                ...
                drainBaselineZFt = drainIndices.Average(idx => vertices[idx].Position.Z);

                // ── Main slope loop (O(V) with precomputed distances) ────
                for (int i = 0; i < vertices.Count; i++)
                {
