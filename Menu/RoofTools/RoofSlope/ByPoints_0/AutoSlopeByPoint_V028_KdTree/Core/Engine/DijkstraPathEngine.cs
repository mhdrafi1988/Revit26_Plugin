...
namespace Revit26_Plugin.AutoSlopeByPointKdTree.VKD01.Core.Engine
{
    public class DijkstraPathEngine
    {
            ...
            double curveTolFt = 0.0033) // ~1mm default
        {
            ...
            _spatialIndex = new KdTree3D(positions);
        ...
        private void BuildGraph()
        {
            ...
            // Candidate pairs are found via KD-tree radius search instead of
            // an O(n^2) all-pairs scan: only vertices within _edgeThresholdFt
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                candidates.Clear();
                _spatialIndex.RangeSearch(a, _edgeThresholdFt, candidates);

                foreach (int j in candidates)
                {
                    if (j <= i) continue; // undirected edge already handled (or self-match)
