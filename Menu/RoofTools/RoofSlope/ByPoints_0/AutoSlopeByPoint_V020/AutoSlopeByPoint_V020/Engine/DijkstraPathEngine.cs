...
namespace Revit26_Plugin.AutoSlopeByPoint.V020.Core.Engine
{
    ...
    public class DijkstraPathEngine
    {
        ...
        /// <summary>
        /// For edges classified as TangentRoute, stores the two tangent points
        /// (ptA near vertex i's side, ptB near vertex j's side) chosen as the
        /// cheapest detour around the crossing arc. Populated during BuildGraph,
        /// alongside _edgeType. Used by CountDistinctTangentPoints to report the
        /// "Tangent Curve Count" summary metric — deduped points actually used
        /// on final paths, not raw hop count.
        /// </summary>
        private readonly Dictionary<(int, int), (XYZ ptA, XYZ ptB)> _tangentPoints = new();
                ...
                for (int j = i + 1; j < n; j++)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            var tangentResult = TryComputeTangentPathWeight(a, b, crossingArcs);
                            if (tangentResult == null) continue;
                            weight = tangentResult.Value.weight;
                            type = EdgeRouteType.TangentRoute;
                            _tangentPoints[(i, j)] = (tangentResult.Value.ptA, tangentResult.Value.ptB);
                            _tangentPoints[(j, i)] = (tangentResult.Value.ptB, tangentResult.Value.ptA);
                        }
                    }
        ...
        /// </summary>
        private (double weight, XYZ ptA, XYZ ptB)? TryComputeTangentPathWeight(XYZ a, XYZ b, List<Arc> crossingArcs)
        {
            (double weight, XYZ ptA, XYZ ptB)? best = null;
            ...
            foreach (Arc arc in crossingArcs)
            {
                ...
                foreach (var tA in tangentsA)
                {
                    ...
                    foreach (var tB in tangentsB)
                    {
                        ...
                        if (best == null || total < best.Value.weight)
                            best = (total, ptA, ptB);
                    }
                }
            }
        ...
        /// <summary>
        /// Walks the predecessor chain from vertexIndex (via _lastPred, from the
        /// most recent ComputeAllDistances call) and returns how many TangentRoute
        /// hops were used on that specific path. Used by AutoSlopeEngine to
        /// accumulate the "Curve Tangents Added" (total hop count) and
        /// "Tangent Path Used" (count of vertices with >=1 tangent hop) summary
        /// metrics. Zero extra Dijkstra cost — reuses the already-computed
        /// predecessor array from ComputeAllDistances.
        /// </summary>
        public int CountTangentHops(int vertexIndex)
        {
            if (_lastPred == null) return 0;
            ...
            {
                int prev = _lastPred[cur];
                if (_edgeType.TryGetValue((cur, prev), out var t) && t == EdgeRouteType.TangentRoute)
                    count++;
                cur = prev;
            }
        ...
        /// <summary>
        /// Walks the predecessor chain from vertexIndex (via _lastPred) and
        /// collects the tangent points used on any TangentRoute hops along that
        /// path. Points are XYZ coordinates, not deduped here — the caller
        /// (AutoSlopeEngine) accumulates these across all vertices into a
        /// tolerance-based deduped set for the "Tangent Curve Count" metric,
        /// since the same physical tangent point can be reused by more than
        /// one vertex's path and should only count once.
        /// </summary>
        public List<XYZ> GetTangentPointsOnLastPred(int vertexIndex)
        {
            var pts = new List<XYZ>();
            if (_lastPred == null) return pts;
            ...
            while (_lastPred[cur] != -1 && guard++ < _verts.Count + 5)
            {
                int prev = _lastPred[cur];
                if (_edgeType.TryGetValue((cur, prev), out var t) && t == EdgeRouteType.TangentRoute
                    && _tangentPoints.TryGetValue((cur, prev), out var pair))
                {
                    pts.Add(pair.ptA);
                    pts.Add(pair.ptB);
                }
