...
// Namespace: Revit26_Plugin.AutoSlopeByPoint.V020
// Changes vs V06:
...
using Revit26_Plugin.AutoSlopeByPoint.V020.Core.Models;
using Revit26_Plugin.AutoSlopeByPoint.V020.Core.Parameters;
using Revit26_Plugin.AutoSlopeByPoint.V020.Infrastructure.Helpers;
...
namespace Revit26_Plugin.AutoSlopeByPoint.V020.Core.Engine
{
    public static class AutoSlopeEngine
    {
        public static void Execute(UIApplication app, AutoSlopePayload data)
        {
            ...
            int tangentHopsTotal = 0;      // sum of TangentRoute hops across all paths
            int tangentPathVertexCount = 0; // count of DISTINCT vertices using >=1 tangent hop
            var distinctTangentPoints = new List<XYZ>(); // deduped tangent points (curveTolFt tolerance)
            double maxPathFt = 0;
            ...
            using (Transaction tx = new Transaction(doc, "Apply AutoSlope"))
            {
                ...
                for (int i = 0; i < vertices.Count; i++)
                {
                    ...
                    // Curve/tangent summary: only meaningful when curve handling
                    // is enabled (boundaryArcs populated) — same gating as the
                    // ARC-DEBUG diagnostic above. Reuses the predecessor chain
                    // already computed by ComputeAllDistances, at zero extra cost.
                    if (data.InsertCurveIntersectionPoints && boundaryArcs != null && boundaryArcs.Count > 0)
                    {
                        int hopsOnThisPath = dijkstra.CountTangentHops(i);
                        if (hopsOnThisPath > 0)
                        {
                            tangentHopsTotal += hopsOnThisPath;
                            tangentPathVertexCount++;

                            // Dedupe against points already seen from OTHER vertices'
                            // paths — the same physical tangent point is frequently
                            // reused across nearby vertices, and should only count
                            // once for "Tangent Curve Count" (distinct points, not hops).
                            foreach (XYZ pt in dijkstra.GetTangentPointsOnLastPred(i))
                            {
                                bool alreadySeen = false;
                                foreach (XYZ existing in distinctTangentPoints)
                                    if (existing.DistanceTo(pt) <= curveTolFt) { alreadySeen = true; break; }
                                if (!alreadySeen) distinctTangentPoints.Add(pt);
                            }
                        }
                    }
            ...
            {
                data.Log?.Invoke(new LogEntry(LogLevel.Info,
                    $"Curves Found             : {boundaryArcs?.Count ?? 0}"));
                data.Log?.Invoke(new LogEntry(LogLevel.Info,
                    $"Tangent Added            : {tangentHopsTotal}"));
                data.Log?.Invoke(new LogEntry(LogLevel.Info,
                    $"Tangent Path Used        : {tangentPathVertexCount}"));
                data.Log?.Invoke(new LogEntry(LogLevel.Info,
                    $"Tangent Curve Count      : {distinctTangentPoints.Count}"));
            }
            ...
            data.OnCompleted?.Invoke(new AutoSlopeResult
            {
                ...
                CurvesFound       = boundaryArcs?.Count ?? 0,
                TangentAdded      = tangentHopsTotal,
                TangentPathUsed   = tangentPathVertexCount,
                TangentCurveCount = distinctTangentPoints.Count
            });
        }
