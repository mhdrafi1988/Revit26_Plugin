...
namespace Revit26_Plugin.AutoSlopeByPointRidge.V001.Core.Engine
{
    public static class CircleMarkerService
    {
        public class PlacementCounts
        {
            ...
            public int RidgeCirclesPlaced { get; set; }
        }

        /// <summary>
        /// Places circles for all four marker groups (Drain / Highest / Allowed
        /// Offset / Ridge). Must be called inside an already-open Transaction.
        /// Returns per-group placed counts (zero for any group that was disabled,
        /// had no qualifying points, or was skipped due to an invalid active view).
        /// </summary>
            ...
            Action<LogEntry> log)
        {
            ...
            // ── Group 4: Ridge Points (V001) ─────────────────────────────────
            // One circle per vertex the ridge rule applied to. Not de-duplicated
            // against the other groups: a ridge point is very often also the
            // highest point, and seeing both rings there is the useful signal.
            if (ridgeGroup != null && ridgeGroup.IsEnabled)
            {
                var ridgeVertices = (vertexDataList ?? new List<VertexData>())
                    .Where(v => v.WasProcessed && v.IsRidgePoint)
                    .ToList();

                if (ridgeVertices.Count == 0)
                {
                    log?.Invoke(new LogEntry(LogLevel.Info,
                        "Circle Markers: Ridge group enabled but no ridge points were found."));
                }
                else
                {
                    var clusteredRidgePoints = ClusterPoints(
                        ridgeVertices.Select(v => v.Position).ToList(), clusterToleranceFt);
                    int placed = 0;
                    foreach (var pt in clusteredRidgePoints)
                    {
                        if (PlaceOneCircle(doc, activeView, pt, viewElevFt, ridgeGroup, log))
                            placed++;
                    }
                    counts.RidgeCirclesPlaced = placed;
                    log?.Invoke(new LogEntry(LogLevel.Success,
                        $"Circle Markers: placed {placed} ridge circle(s) for {ridgeVertices.Count} ridge point(s)."));
                }
            }
