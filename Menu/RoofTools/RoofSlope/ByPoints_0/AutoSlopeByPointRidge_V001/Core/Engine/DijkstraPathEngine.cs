...
namespace Revit26_Plugin.AutoSlopeByPointRidge.V001.Core.Engine
{
    public class DijkstraPathEngine
    {
        ...
        // ── Graph access (Ridge V001) ────────────────────────────────────────
        // RidgeEngine needs to walk the same graph the distances were computed
        // on (to find cross-basin edges), so the adjacency and edge weights are
        // exposed read-only here instead of being rebuilt elsewhere.

        public int VertexCount => _verts.Count;

        /// <summary>Graph neighbours of vertex <paramref name="i"/> (valid edges only).</summary>
        public IReadOnlyList<int> Neighbors(int i) =>
            _adj.TryGetValue(i, out var list) ? list : (IReadOnlyList<int>)Array.Empty<int>();

        /// <summary>Edge weight (internal feet, arc-aware) between two adjacent vertices.</summary>
        public double EdgeWeight(int i, int j) => Weight(i, j);
        ...
        /// <summary>
        /// Lower-bounded multi-source Dijkstra (Ridge V001).
        ///
        /// Works directly in ELEVATION units (feet of rise) instead of path length:
        /// every drain starts at 0, and crossing an edge of length w costs
        /// w × slopeFactor. On top of that, each vertex may carry a minimum
        /// elevation (<paramref name="minElevFt"/>, the ridge lift). Relaxation is
        ///     e(nb) = max( minElev(nb), e(v) + w × slopeFactor )
        /// which is monotone non-decreasing along any path, so plain Dijkstra
        /// label-setting stays exact.
        ///
        /// Result guarantees:
        ///   • every reachable non-drain vertex has at least one neighbour it can
        ///     descend to at ≥ the given slope (its predecessor) — no ponds;
        ///   • ridge vertices sit at or above their lift;
        ///   • a vertex whose only way down passes a lifted ridge is itself lifted
        ///     (e(ridge) + slope × w), OR — if some other drain group is cheaper —
        ///     it simply re-flows to that group at exactly the given slope. This
        ///     is the "move watershed" behaviour: everything behind a ridge drains
        ///     to the far drain.
        ///
        /// <paramref name="sourceGroup"/> receives, per vertex, the drain-group index
        /// of the source that set its final label (informational: which drain the
        /// vertex ultimately flows to). Drains keep their own group.
        /// </summary>
        public double[] ComputeLowerBoundedElevations(
            IReadOnlyDictionary<int, int> drainToGroup,
            double[] minElevFt,
            double slopeFactor,
            out int[] sourceGroup)
        {
            ...
            sourceGroup = new int[n];
            for (int i = 0; i < n; i++)
            {
                elev[i] = double.PositiveInfinity;
                sourceGroup[i] = -1;
            }
            ...
            foreach (var kv in drainToGroup)
            {
                int drain = kv.Key;
                if (drain < 0 || drain >= n) continue;
                elev[drain] = 0;
                sourceGroup[drain] = kv.Value;
                pq.Add((0, drain));
            }
            ...
            while (pq.Count > 0)
            {
                var (e, v) = pq.Min;
                pq.Remove(pq.Min);
                if (e > elev[v]) continue; // stale entry

                foreach (int nb in _adj[v])
                {
                    double candidate = e + Weight(v, nb) * slopeFactor;
                    double floor = (minElevFt != null && nb < minElevFt.Length) ? minElevFt[nb] : 0;
                    if (floor > candidate) candidate = floor;

                    if (candidate < elev[nb])
                    {
                        elev[nb] = candidate;
                        sourceGroup[nb] = sourceGroup[v];
                        pq.Add((candidate, nb));
                    }
                }
            }
