            ...
            List<RidgeJunctionResult> ridgeJunctionResults = null)
        {
            ...
            try
            {
                return ExcelExportHelper.ExportCompactVertexData(
                    payload, vertexData, roof, slopePercent, version, status, ridgePairResults, ridgeJunctionResults);
            }
            catch (Exception ex)
            {
            ...
            List<RidgeJunctionResult> ridgeJunctionResults = null)
        {
            ...
            try
            {
                return ExcelExportHelper.ExportDetailedVertexData(
                    payload, vertexData, roof, drainPoints, slopePercent, ridgePairResults, ridgeJunctionResults);
            }
            catch (Exception ex)
            {
                payload?.Log?.Invoke(new LogEntry(LogLevel.Error,
