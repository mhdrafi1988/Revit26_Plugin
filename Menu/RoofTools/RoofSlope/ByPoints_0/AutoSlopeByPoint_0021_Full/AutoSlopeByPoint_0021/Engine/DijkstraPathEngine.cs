...
namespace Revit26_Plugin.AutoSlopeByPoint.V021.Core.Engine
{
    ...
    public class DijkstraPathEngine
    {
        ...
        /// <summary>
        /// Same multi-source Dijkstra as ComputeAllDistances, but additionally
        /// returns WHICH source vertex (from the input 'drains' set) produced each
        /// vertex's shortest distance — needed by the "Max Demand" ridge elevation
        /// method (V021), which needs the actual Z of the specific nearest drain
        /// point per group, not just the group's overall nearest-distance value.
        /// Kept as a separate method (not a signature change to the existing
        /// ComputeAllDistances) so no other caller is affected.
        /// </summary>
        public (double[] Distances, int[] SourceVertex) ComputeAllDistancesWithSource(HashSet<int> drains)
        {
            int n = _verts.Count;
            var dist = new double[n];
            var pred = new int[n];
            ...
            foreach (int drain in drains)
            {
                dist[drain] = 0;
                pred[drain] = -1;
                source[drain] = drain; // a drain vertex's own source is itself
                pq.Add((0, drain));
            }
            ...
            while (pq.Count > 0)
            {
                ...
                foreach (int nb in _adj[v])
                {
                    ...
                    if (nd < dist[nb])
                    {
                        dist[nb] = nd;
                        pred[nb] = v;
                        source[nb] = source[v]; // inherit the originating drain from whichever neighbor reached it
                        pq.Add((nd, nb));
                    }
                }
            }
