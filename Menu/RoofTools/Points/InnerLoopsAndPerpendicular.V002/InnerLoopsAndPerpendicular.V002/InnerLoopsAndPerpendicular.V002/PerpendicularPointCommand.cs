...
using Revit26_Plugin.DivideInnerLoopsAndPerpendicular.V002.Views;
using System;

namespace Revit26_Plugin.DivideInnerLoopsAndPerpendicular.V002
{
    /// <summary>
    /// External command that launches the Perpendicular Border Point tool V001.
    ...
    public class PerpendicularPointCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                var vm = new PerpendicularPointViewModel(doc, roof);
                vm.AnalyzeCommand.Execute(null);

                // ── Launch UI ────────────────────────────────────────────────
                var window = new PerpendicularPointWindow
                {
                    DataContext = vm,
                    Topmost     = true
                };
