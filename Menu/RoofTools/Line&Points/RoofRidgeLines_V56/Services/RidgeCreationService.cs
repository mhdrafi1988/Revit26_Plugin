...
namespace Revit26_Plugin.RoofTools.LineAndPoints.RoofRidgeLines.V56.Services
{
    public class RidgeCreationService
    {
            ...
            VoronoiRidgeResult result)
        {
            ...
            // TX-0: Detail lines. Built only from result.ClippedEdges, so it
            // has no dependency on the slab shape editor — it now runs FIRST,
            // as a visual fallback that is guaranteed even if shape editing
            // can't be enabled afterward.
            ExecuteTx1_DetailLines(doc, activeView, result);

            // TX-1: Enable shape editing — runs second (after the detail-line
            // fallback is already committed) so everything else can assume a
            // known editor state before doing any further work.
            ...
            ExecuteTx2_AddPoints(doc, roof, result);
            OnPointsDone?.Invoke();
        ...
        // ── TX-1: Enable (runs second, after detail lines are committed) ───────────
        ...
        // ── TX-2: AddSplitLine + AddPoint (unchanged except for tracking) ─────────
            ...
            List<XYZ> existingVertices)
        {
            try
            {
                ...
                // Get all current vertices after TX-2
                ...
                foreach (var v in allVertices)
                {
                    ...
                    // Check if this vertex existed before TX-2
        ...
        // ── TX-0: Detail lines (runs first in CreateAll, before shape editing ──────
        //         is enabled; built only from result.ClippedEdges, so it always
        //         runs as a visual fallback even if TX-1 / the model update fails) ──
