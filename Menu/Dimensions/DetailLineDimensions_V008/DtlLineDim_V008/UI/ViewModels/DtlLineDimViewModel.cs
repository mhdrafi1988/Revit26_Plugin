...
using Revit26_Plugin.DtlLineDim.V008.Infrastructure.ExternalEvents;
using Revit26_Plugin.Shared.Models;
using Revit26_Plugin.Shared.Services;

namespace Revit26_Plugin.DtlLineDim.V008.UI.ViewModels
{
    public partial class DtlLineDimViewModel : ObservableObject
    {
        ...
        /// <summary>
        /// Restores the last-used Detail Item Type / Dimension Type by name from
        /// settings.json. If no saved name exists, or the saved name no longer
        /// matches anything in the current model (renamed/deleted type), falls
        /// back to the first item in the respective list — added in V008 so the
        /// tool never opens with an unselected ComboBox when items are available.
        /// </summary>
        public void RestoreLastSelections()
        {
            ComboItem restoredDetailItemType = !string.IsNullOrEmpty(_settings.LastDetailItemTypeName)
                ? DetailItemTypes.FirstOrDefault(i => i.Name == _settings.LastDetailItemTypeName)
                : null;
            SelectedDetailItemType = restoredDetailItemType ?? DetailItemTypes.FirstOrDefault();
            Log(LogLevel.Info, restoredDetailItemType != null
                ? $"Detail Item Type restored from settings: '{SelectedDetailItemType?.Name}'"
                : $"Detail Item Type defaulted to first item: '{SelectedDetailItemType?.Name ?? "none available"}'");

            ComboItem restoredDimensionType = !string.IsNullOrEmpty(_settings.LastDimensionTypeName)
                ? DimensionTypes.FirstOrDefault(i => i.Name == _settings.LastDimensionTypeName)
                : null;
            SelectedDimensionType = restoredDimensionType ?? DimensionTypes.FirstOrDefault();
            Log(LogLevel.Info, restoredDimensionType != null
                ? $"Dimension Type restored from settings: '{SelectedDimensionType?.Name}'"
                : $"Dimension Type defaulted to first item: '{SelectedDimensionType?.Name ?? "none available"}'");
        }
